...
export class JobBusyError extends Error {
  constructor() {
    super("Já existe um trabalho rodando. Espere ele terminar ou cancele.");
 ...
 *
 * Concurrency is refused by `jobs_one_running_idx`, a partial unique index, so
 * two fast clicks cannot both win — a JavaScript guard would lose that race.
 */
export function startJob(
  ...
  try {
    const [row] = db
      .insert(jobs)
      .values({ kind, projectId, status: "running", progress: { done: 0, total: 0 } })
...
export function currentJob(db: Db) {
  const [row] = db.select().from(jobs).where(eq(jobs.status, "running")).limit(1).all();
  return row ?? null;
}
